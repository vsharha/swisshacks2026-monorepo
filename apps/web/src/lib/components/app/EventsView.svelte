...
<div class="border-line bg-panel flex min-w-0 shrink-0 flex-col rounded-lg border p-4">
	<div class="text-muted2 mb-2 text-[10px] tracking-[0.16em] uppercase">
		Events · {events.length}
	</div>
	<div class="[&>[data-slot=table-container]]:overflow-x-visible">
			...
			<Table.Body>
				{#each events as s (s.id)}
					...
					<Table.Row class="divide-line/70 border-line align-top">
						<!-- Event -->
						<Table.Cell class="py-1.5 align-top whitespace-normal">
							<div class="flex items-start gap-2.5">
								...
								<div class="flex min-w-0 flex-col gap-0.5">
									<span class="text-muted2 font-mono text-[10px]"
										>{fmtDate(s.date)} · {s.axis} · {s.type.replace(/_/g, ' ')}</span
									>
						...
						<!-- Confidence -->
						<Table.Cell class="py-1.5 align-top">
							<span
						...
						<!-- Market research -->
						<Table.Cell class="text-muted2 py-1.5 align-top leading-snug whitespace-normal">
							{deriveMarketResearch(s)}
						</Table.Cell>

						<!-- Signal inference (short; full read on hover) -->
						<Table.Cell class="text-text2 py-1.5 align-top leading-snug whitespace-nowrap">
